...
function createVotingRoot (container: HTMLElement, initialState: any) {
  ...
  function render (state: any) {
    ...
    try {
      if (typeof votingProps.choices === 'string') votingProps.choices = JSON.parse(votingProps.choices)
    } catch (e) {
      votingProps.choices = []
    }
...
export default function init () {
  ...
  function requestResize () {
    try {
      const height = document.body.scrollHeight || (votingContainer ? votingContainer.scrollHeight : 300)
      postAction({
